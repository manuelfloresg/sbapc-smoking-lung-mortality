...
OUT_DIR <- "results/20260506_1420_FIX_MORT_INC_V6"
...
plot_deconstruction_fixed <- function(scen, bench_m, seed=4, dgp="spec_linear") {
  ...
  ggplot(plot_df, aes(x = period, y = Deaths, color = Series, linetype = Series)) +
    ...
    labs(title = paste("Mortality - Scenario:", scen), subtitle = "CLEAN BASELINE RATE (V6 FINAL)") + theme_minimal()
}
...
for (scen in c("quit", "up1pc", "down1pc", "freeze")) {
  ggsave(file.path(OUT_SEC4, sprintf("fig_final_v6_mort_%s.png", scen)), plot_deconstruction_fixed(scen, bench_mort), width=10, height=6, bg="white")
  ggsave(file.path(OUT_SEC4, sprintf("fig_final_v6_inc_%s.png", scen)), plot_incidence_fixed(scen, bench_inc), width=10, height=6, bg="white")
}
message("DONE")
